...
CREATE INDEX IF NOT EXISTS idx_scheduled_task_due ON scheduled_task(status, completed, executing, next_run_at);
`
...
func openDB() (*sql.DB, error) {
	taskDBOnce.Do(func() {
		...
		if _, err := db.Exec(scheduledTasksTableV2); err != nil {
			_ = db.Close()
			taskDBErr = err
			return
		}
		// Migrate older DBs safely (no silent failures).
...
func ensureScheduledTaskSchema(db *sql.DB) error {
	...
	for _, d := range defs {
		...
		logging.Info("scheduled_task migrated: added column %s", d.name)
	}
