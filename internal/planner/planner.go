...
func DoExe(ctx context.Context, rp string) (string, error) {
	...
	outputchan, ok := ctx.Value(utils.DPDialogOutputChanString).(chan string)
	if !ok {
		logging.Error("Failed to get outputchan from context")
		return "", fmt.Errorf("Failed to get outputchan from context")
	}
			...
			if p.Steps[i].Result != nil {
				...
				if resultStr, ok := p.Steps[i].Result.(string); ok {
					...
					if err := json.Unmarshal([]byte(resultStr), &parsedResult); err != nil {
						logging.Error("Failed to parse result JSON: %v", err)
						return "", err
					}
					p.Steps[i].Result = parsedResult
					outputchan <- "执行步骤" + p.Steps[i].Id + "完成，以下是结果：" + resultStr
					memory.AddUserMessage(chatID, "我已经执行了步骤"+p.Steps[i].Id+":"+p.Steps[i].Tool+"，以下是结果："+resultStr)
				}
			}
		}

		resultSteps, err := json.MarshalIndent(p, "", "  ")
		if err != nil {
			logging.Error("Failed to marshal plan results: %v", err)
			return "", err
		}
		logging.Info("Plan results: %s", string(resultSteps))

	}
	return string(resultSteps), nil
}

func ParsePlan(jsonStr string) (*Planning, error) {
	var planData struct {
