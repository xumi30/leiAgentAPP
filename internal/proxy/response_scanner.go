...
func (p *Client) handleStreamResponse(ctx context.Context, resp *http.Response) (*ToolAndContent, error) {
	...
	dialogMessageID := utils.GenerateMessageID()
	reasoningMessageID := utils.GenerateMessageID()
	...
	for scanner.Scan() {
		...
		if ok && content != "" {
			...
			if outgoingContent != "" {
				...
				globalchannel.SendAssistantMessageStream(ctx, outgoingContent, dialogMessageID, false, 0)
			}
		...
		if reasoningContent != "" {
			...
			globalchannel.SendReasoningMessageStream(ctx, reasoningContent, reasoningMessageID, false)
		}
	}
	...
	if stripNeedActionHeader && !needActionHeaderParsed && needActionHeaderBuffer.Len() > 0 {
		...
		globalchannel.SendAssistantMessageStream(ctx, bufferedContent, dialogMessageID, false, 0)
	}

	result := fullContent.String()
	reasoningResult := fullReasoningContent.String()

	// 关键：发送一次“流结束”信号（IsFinished=true），让 AppendAgentMessageToFrontRole 做收口入库并 emit dialogStreamEnd。
	// 否则前端虽然能看到流式拼接的内容，但刷新后因 DB 未落库而消失。
	globalchannel.SendAssistantMessageStream(ctx, "", dialogMessageID, true, effectiveUsageTotal(lastUsage))
	globalchannel.SendReasoningMessageStream(ctx, "", reasoningMessageID, true)
...
func (p *Client) handleNonStreamResponse(ctx context.Context, resp *http.Response) (*ToolAndContent, error) {
	...
	if !skipDialog {
		if strings.TrimSpace(content) != "" {
			globalchannel.SendAssistantMessageOnce(ctx, content, effectiveUsageTotal(openaiResp.Usage))
		}
	}
